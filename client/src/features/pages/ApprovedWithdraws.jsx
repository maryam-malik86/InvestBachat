import React from 'react'
import Navbar from '../dashboard/DashboardComponents/Navbar'
import LeftSideBar from '../dashboard/DashboardComponents/LeftSideBar'
import {useGetAllApprovedWithdrawalsQuery} from '../Admin side/ApprovingReceiptsApi'
import { PropagateLoader } from "react-spinners";
import {useNavigate} from 'react-router-dom'
const ApprovedWithdraws = () => {
  const {data,isLoading} = useGetAllApprovedWithdrawalsQuery();
  const navigate = useNavigate();

  return (
    <div>
      <Navbar/>
      <LeftSideBar/>
      {
        isLoading ? (
          <div className="flex justify-center items-center h-[full]">
          <PropagateLoader color="#3B82F6" />
        </div>
        ):(
          <div className='mt-20 min-h-[10rem]'>
  <div className='xl:pl-[16rem] mx-auto xl:w-[96%] w-[95%] mt-[7.8rem] min-h-[10rem]'>
  {data && data.length > 0 ? (
    data.map((approvedWithdraws) => (
      <div className="my-4" key={approvedWithdraws._id}>
        <div className="flex justify-between items-center pl-5 bg-gray-200 w-[100%] rounded-lg">
          <div className="w-[60%] flex justify-between">
            <div>{approvedWithdraws.user_name}</div>
            <div className="md:block hidden">{approvedWithdraws.user_cnic}</div>
          </div>

          <div className="flex gap-2">
            <button
              className="h-[3rem] w-[5rem] bg-indigo-500 text-white rounded-lg"
              onClick={() => {
                navigate(`/admin/getApprovedWithdrawalById/${approvedWithdraws._id}`);
              }}
            >
              Approved
            </button>
          </div>
        </div>
      </div>
    ))
  ) : (
    <div className="flex flex-col justify-center items-center h-screen text-2xl font-bold text-indigo-500 space-y-4">
      <svg
        xmlns="http://www.w3.org/2000/svg"
        fill="none"
        viewBox="0 0 24 24"
        stroke="currentColor"
        className="h-12 w-12 text-indigo-500"
      >
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
      </svg>
      No pending left
    </div>
  )}
</div>
          </div>
        )
      }
    </div>
  )
}

export default ApprovedWithdraws
